...
import com.example.layeredarchitecture.view.dao.custom.impl.QueryDAOImpl;
...
public class MainFormController {
    ...
    private void navigate(MouseEvent event) throws IOException, SQLException, ClassNotFoundException {
        if (event.getSource() instanceof ImageView) {
            ...
            switch (icon.getId()) {
                case "imgCustomer":
                    root = FXMLLoader.load(this.getClass().getResource("/lk/grb/layeredarchitecture/manage-customers-form.fxml"));
                    break;
                case "imgItem":
                    root = FXMLLoader.load(this.getClass().getResource("/lk/grb/layeredarchitecture/manage-items-form.fxml"));
                    break;
                case "imgOrder":
